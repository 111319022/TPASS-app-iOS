...
- 站點資料已全面改為 JSON 驅動，來源以 TDX 為主。
- 票價資料中，北捷使用台北資料大平台（Data Taipei），其餘運具以 TDX 為主。
- 台鐵、高鐵目前仍維持既有資料庫與查價服務，未納入這次 JSON 化範圍。
- 新版票價（TDX） JSON 會以 `TicketType = 1`、`FareClass = 1` 作為正式單程票查價依據。
- 語音解析規則由 `Data/VoiceNLP/VoiceNLP_Rules.json` 提供（關鍵字、ASR 誤聽字、站名別名、正則、信心權重）。
...
- 回家站點與出門站點快捷新增
- 快捷新增（含單筆新增）若日期超出當前週期，會先阻擋儲存並提示調整時間
- 語音快速新增：可直接說出日期/時間/運具/起訖站/路線，進入預覽後一鍵儲存
- 語音解析會自動正規化站名、回填捷運線別、並嘗試自動查價
- 台鐵語音新增會檢查月票可用站點範圍，超出時會提示並阻擋儲存
- 語音儲存成功後會背景上傳匿名修正紀錄（VoiceParseLog），用於後續 NLP 規則優化
- 支援票卡管理（TPASS 卡 / 自訂卡）與卡片資料維護頁

### TPASS 與轉乘優惠計算
- 依「方案 + 身分 + 轉乘型態」自動換算實付金額
- 支援基隆、雙北、桃園、宜蘭、台中、高雄、台南、嘉義等跨區規則
- 支援彈性記帳週期（可選運具子集合）
- 台鐵區間合法性與多區段範圍對應（依方案）
- 票價查詢大部分已改為 JSON 載入，站點與票價資料不再寫死在程式碼中
...
- 開發者可於 App 內查看回報清單、狀態篩選（待處理/已修復）與回報詳情
- 開發者可在回報詳情中標記「已修復」，並可於清單/詳情刪除回報
- 點擊回報通知可自動導頁至對應回報詳情
- 開發者工具提供「清除所有標記」，可清除回報通知與 App 角標
- CloudKit Public Database 的管理寫入操作需要 Admin 身分（至少包含標記回報完成狀態）
- 開發者工具支援匯出 `VoiceParseLog` CSV（最多 500 筆），便於離線分析語音解析準確率與規則調校

### 使用體驗
- 通知系統：每日提醒、週期到期提醒
- 主題切換：系統/亮色/深色/暖色（Muji）/紫色（Purple）
- Spotlight 教學與引導頁
...
語音快速新增已完整落地，流程為「語音輸入 -> 轉寫 -> 解析（支援多段/轉乘）-> 預覽修正 -> 批次建立 Trip」。
以下列出目前已實作的核心檔案與責任：

- `Views/VoiceQuickTripView.swift`
    - 語音快速記錄主畫面與狀態流程（錄音、解析、預覽、手動修正、儲存）
    - V2 支援多段行程（轉乘）：時間軸預覽、可新增/刪除段落、批次儲存多筆 Trip
    - 解析結果回填站名/線別/地區資料，並觸發自動查價（必要時帶入區域預設公車票價）
    - 台鐵儲存前會檢查月票可用站點範圍，超出時提示並阻擋儲存
    - 儲存成功／解析失敗／使用者放棄儲存皆會呼叫 `VoiceParseLogService` 上傳匿名紀錄（`success/failed/abandoned`）
- `Views/VoiceSegmentEditorCard.swift`
    - 多段行程預覽時每一段的編輯卡片（運具、起訖站、路線、票價、日期、轉乘/免費/高鐵自由座）
    - 顯示解析信心提示，並在欄位變更時觸發自動查價
- `Services/VoiceInputService.swift`
    - 語音辨識輸入控制（啟動、停止、權限、轉錄文字）
    - 內建 55 秒保護（避免 iOS 語音辨識時長限制造成異常）
- `Services/TripVoiceParser.swift`
    - 將轉錄文字解析為結構化欄位（運具、起訖站、路線、日期時間、票價），並計算信心分數
    - 規則由 JSON 載入（Lazy Loading），支援優先級消歧、ASR 誤聽容錯、信心分數配置化
    - V2 支援多段行程切割、上下文補漏（下一段缺起點時帶入上一段終點）、第二段起自動標記轉乘
- `Models/VoiceDraft.swift`
    - 語音解析中介模型（ParsedTrip -> VoiceDraft），供 UI 預覽與修正
- `Services/VoiceParseLogService.swift`
    - 匿名上傳語音解析紀錄到 CloudKit Public Database（Record Type: `VoiceParseLog`）
    - 會過濾 userId/appleId/deviceId/email 等敏感欄位
- `Services/VoiceLogExportService.swift`
    - 從 CloudKit Public Database 匯出 `VoiceParseLog` 為 CSV（最多 500 筆），供離線分析與規則調校
- `Data/VoiceNLP/VoiceNLP_Rules.json`
    - 結構化 NLP 規則庫（noise_filters、transports、station_aliases、patterns、confidence）
    - 可透過調整 JSON 迭代規則，避免頻繁修改 Swift 程式碼
- `Services/FareServices/*` 與 `Services/New_FareServices/*`
    - 依運具進行票價查詢，補齊語音解析未明確給出的票價

如果你要改語音功能，建議優先從 `VoiceQuickTripView`、`VoiceSegmentEditorCard` 與 `TripVoiceParser` 開始追。
...
- 若需在 Production 修改 Public Database 回報狀態，測試帳號需具備 CloudKit Admin 角色，否則會出現 `WRITE operation not permitted`
