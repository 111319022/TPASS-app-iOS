...
- **關鍵修正**：站名抽取前會先移除所有時間表達式（日期、時段、時間等），避免「到科技大樓今天下午 3:30」中的時間被誤認為站名。
  - 移除模式包括：相對日期+時段+時間、純時間、月日格式等（見 `removeTimeExpressions()` 函式）。
  - 此修正確保時間資訊後置時，站點名稱仍能正確提取。
- 站名會經過別名/正規化處理，再套用各運具站點驗證。
- **公車路線抽取支援多種說法**（按優先序）：
  - 「284公車」「M101客運」（數字+運具名直接連接，優先級最高）
  - 「295號公車」「M157客運」（數字+路/號+運具名）
  - 「公車 207」「客運 1815」（運具名+數字）
  - 「207路」「1815號」（數字+路/號）
  - 已排除時間格式（例如 12:30）誤判
- 語音回填後會嘗試補齊線別與自動查價。
